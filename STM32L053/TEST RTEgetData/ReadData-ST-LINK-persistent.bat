...
REM after the RTEgetData utility connects to the GDB server.
REM This is not required for the J-Link GDB Server.

"c:\RTEdbg\UTIL\RTEgetData\RTEgetData.exe" 61234 0x20000000 0 -p -decode=Decode.bat -filter=0xFFFFFFFF -start=continue.cmd -priority -driver=stlinkserver.exe -driver="ST-LINK_gdbserver.exe"
